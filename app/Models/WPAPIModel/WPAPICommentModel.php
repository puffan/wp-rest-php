...
class WPAPICommentModel{
    ...
    public function getParentCommentList( $postId , $offset , $limit , $order ){
        $sql = 'select comment_ID as id, comment_post_ID as post, 0 as parent, user_id as author, \'author_name\' as author_name, comment_date_gmt as date, comment_content as content, comment_approved as status, \'test1\' as accountid  from wp_2_comments '.
    ...
    public function getParentCommentCount( $postId ){
        $sql = 'select count(1) as parentCount from wp_2_comments '.
            'where comment_approved=\'1\' and comment_post_id='.$postId.' and comment_type=\'\' and comment_parent=0 '.
            'limit 1; ' ;
        $rs = DB::select( $sql ) ;
        if( !$rs || !$rs[0] ){
    ...
    public function getChildCommentCount( $parentCommentId , $postId ){
        $sql = 'select count(1) as childCount from wp_2_comments '.
            'where comment_approved=\'1\' and comment_post_id='.$postId.' and comment_type=\'\' and comment_parent='.$parentCommentId.' '.
            'limit 1; ' ;
        $rs = DB::select( $sql ) ;
        if( !$rs || !$rs[0] ){
